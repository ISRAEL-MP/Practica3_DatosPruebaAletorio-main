...
class AlbumTestCaseFake(unittest.TestCase):
    ...
    def test_agregar_album_titulo_repetido(self):
        albumTitulo = self.data[ -1 ][ 0 ]
        albumAnio = self.data_factory.albumAnio()
        albumDescripcion = self.data_factory.albumDescripcion()
        albumMedio = self.data_factory.albumMedio()
    ...
    def test_editar_album ( self ) :
        ...
        resultado = self.logica.editar_album(1, albumTitulo, albumAnio, albumDescripcion, albumMedio)
        self.assertEqual(resultado, True)


    def test_editar_album_titulo_repetido(self):
        albumTitulo=self.data[ -1 ][ 0 ]
        albumAnio=self.data_factory.albumAnio ( )
        albumDescripcion=self.data_factory.albumDescripcion ( )
        albumMedio=self.data_factory.albumMedio()
